...
$spkmasuk = query("SELECT * FROM spkmasuk WHERE cek ='0'");
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="table-responsive">
        ...
        <table class="table table-striped table-sm text-center">
          ...
          <?php $i=1; ?>
          <?php foreach ($spkmasuk as $row) : ?>
          <tr>
            <td> <?= $i; ?> </td>
            <td  class="col-sm-1"> <?= $row["jenis"] ?> </td>
            <td> <?= $row["kategori"] ?> </td>
            <td> <?= $row["tanggal"] ?> </td>
            <td> <?= $row["jumlah"] ?> </td>
            <td>
              <a class="btn btn-success btn-sm"href="cekKualitas.php?id=<?= $row["id"];?> " role="button">Cek Kualitas</a>
            </td>
          </tr>
          <?php $i++; ?>
          <?php endforeach; ?>
